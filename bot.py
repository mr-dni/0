...
def search_i(text,chat,bot):
    try:
        ...
        if hasInsult(search)[0] == False and chat['abs_object']['type'] == 'Group':
            bot.sendMessage(chat['object_guid'], 'Full results will be sent to your PV soon {✅}', chat['last_message']['message_id'])                           
            ...
            bot.sendMessage(chat['object_guid'], 'Finding a little patience ....', chat['last_message']['message_id'])
            print('search image')
            jd = json.loads(requests.get('https://zarebin.ir/api/image/?q=' + search + '&chips=&page=1').text)
            jd = jd['results']
            a = 0
        ...
        if chat['last_message']['author_object_guid'] in admins:
            ...
            if 'reply_to_message_id' in msg_data.keys():
                ...
                if not msg_data['author_object_guid'] in admins:
                    ...
                    bot.sendMessage(chat['object_guid'], 'Done {✅}' , chat['last_message']['message_id'])
                    return True
        return False
    except:
        print('server ban bug')
    ...
    try:
        ...
        if hasInsult(search)[0] == False and chat['abs_object']['type'] == 'Group':                               
            ...
            for result in results:
                text += result['title'] + '\n\n'
            bot.sendMessage(chat['object_guid'], 'Full results sent to your PV {✅}', chat['last_message']['message_id'])
            bot.sendMessage(chat['last_message']['author_object_guid'], 'Results found for (' + search + ') : \n\n'+text)
        elif chat['abs_object']['type'] == 'User':
            jd = json.loads(requests.get('https://zarebin.ir/api/?q=' + search + '&page=1&limit=10').text)
            results = jd['results']['webs']
            text = ''
        ...
        bot.sendMessage(chat['object_guid'], 'This command is currently limited or under repair' , chat['last_message']['message_id'])
        return False
...
def get_font(text,chat,bot):
    try:
        ...
        if hasInsult(name_user)[0] == False and chat['abs_object']['type'] == 'Group':
            bot.sendMessage(chat['object_guid'], 'Full results sent to your PV {✅}', chat['last_message']['message_id'])
            bot.sendMessage(chat['last_message']['author_object_guid'], 'Results found for (' + name_user + ') : \n\n'+text)                                        
        elif chat['abs_object']['type'] == 'User':
            bot.sendMessage(chat['object_guid'], text , chat['last_message']['message_id'])
    except:
    ...
    try:
        ...
        if hasInsult(mozoa)[0] == False and chat['abs_object']['type'] == 'Group' and page > 0:
            ...
            if not 'codebazan.ir' in text_t:
                ...
                bot.sendMessage(chat['object_guid'], 'Article "'+ mozoa + '" Page : ' + str(page) + ' Send your PV', chat['last_message']['message_id'])
                bot.sendMessage(chat['last_message']['author_object_guid'], 'Results found for (' + mozoa + ') : \n\n'+text)
...
def get_vaj(text,chat,bot):
    try:
        ...
        if hasInsult(vaj)[0] == False:
            ...
            jd = jd['result']
            text = 'Meaning : \n'+jd['mani'] + '\n\n Definite dictionary:\n' + jd['Fmoein'] + '\n\nDehkhoda dictionary  : \n' + jd['Fdehkhoda'] + '\n\nSynonyms and antonyms : ' + jd['motaradefmotezad']
            bot.sendMessage(chat['object_guid'], text , chat['last_message']['message_id'])
    except:
        print('code bz vaj err')
        
def zekr(text,chat,bot):
    try:                        
...
def get_font_fa(text,chat,bot):
    try:
        ...
        if hasInsult(site)[0] == False and chat['abs_object']['type'] == 'Group':
            bot.sendMessage(chat['object_guid'], 'Full results sent to your PV {✅}', chat['last_message']['message_id'])
            bot.sendMessage(chat['last_message']['author_object_guid'], 'Results found for (' + site + ') : \n\n'+text)                                        
        elif chat['abs_object']['type'] == 'User':
            bot.sendMessage(chat['object_guid'], text , chat['last_message']['message_id'])
    except:
...
def get_added(text,chat,bot):    
    try:
        ...
        send_text = 'Hello dear friend to the group ' + group + ' welcome 🖤✨ \n Robot manufacturer; @e3tefen {🧑🏼‍💻}'
        bot.sendMessage(chat['object_guid'],  send_text, chat['last_message']['message_id'])
    except:
        print('rub server err')
...
g_usvl = ''
test_usvl = ''
auth = "cohraqvmsgedymmhbwufrqlcgumsznap"
bot = Bot(auth)
list_message_seened = []
                    ...
                    if 'SendMessages' in access and chat['last_message']['type'] == 'Text' and text.strip() != '':
                        ...
                        if not m_id in list_message_seened:
                            ...
                            if text == '!start':
                                ...
                                try:
                                    bot.sendMessage(chat['object_guid'], 'Hello\n super bot [XP] Was online \n\n Send the word [!help] For help ',chat['last_message']['message_id'])
                                    print('sended response')    
                                except:
                                    print('server bug1')
                            elif text.startswith('!nim http://') == True or text.startswith('!nim https://') == True:
                                try:
                                    bot.sendMessage(chat['object_guid'], "Preparing link ...",chat['last_message']['message_id'])
                                    print('sended response')
                                    link = text[4:]
                                    ...
                                    nim_baha = pg    
                                    try:
                                        bot.sendMessage(chat['object_guid'], 'Your half price link has been successfully prepared {✅} \n LINK : \n' + nim_baha ,chat['last_message']['message_id'])
                                        print('sended response')    
                                    except:
                                        print('server bug2')
                                except:
                                ...
                                try:
                                    ...
                                    if hasInsult(search)[0] == False and chat['abs_object']['type'] == 'Group':                               
                                        ...
                                        text = ''
                                        for result in results:
                                            if ' - Wikipedia, the free encyclopedia' in result['title']:
                                                title = result['title'].replace(' - Wikipedia, the free encyclopedia','')
                                                text += title + ' :\n\n' + str(result['description']).replace('</em>', '').replace('<em>', '').replace('(Meta Search Engine)', '').replace('&quot;', '').replace(' — ', '').replace(' AP', '') + '\n\nمقاله کامل صفحه 1 : \n' + '!wiki [1:' + title + ']\n\n' 
                                        bot.sendMessage(chat['object_guid'], 'Full results sent to your PV {✅}', chat['last_message']['message_id'])
                                        bot.sendMessage(chat['last_message']['author_object_guid'], 'Results found for (' + search + ') : \n\n'+text)
                                    elif chat['abs_object']['type'] == 'User':
                                ...
                                tawd47 = Thread(target=Time, args=(text, chat, bot,))
                                tawd47.start()
                            elif text.startswith('!hadis'):
                                tawd41 = Thread(target=hadis, args=(text, chat, bot,))
                                tawd41.start()
                            elif text.startswith('!pass'):
                                tawd48 = Thread(target=password, args=(text, chat, bot,))
                                tawd48.start()
                                ...
                                tawd45 = Thread(target=zekr, args=(text, chat, bot,))
                                tawd45.start()
                            elif text.startswith('!dialog'):
                                tawd46 = Thread(target=dialog, args=(text, chat, bot,))
                                tawd46.start()
                                ...
                                try:
                                    ...
                                    if not guid in admins and chat['last_message']['author_object_guid'] in admins:
                                        ...
                                        bot.sendMessage(chat['object_guid'], 'Ban shod {✅}' , chat['last_message']['message_id'])
                                except:
                                    print('ban bug')
                                ...
                                try:
                                    ...
                                    bot.sendMessage(chat['object_guid'], 'Added {✅}' , chat['last_message']['message_id'])                         
                                except:
